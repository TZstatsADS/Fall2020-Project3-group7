...
```{r message=False}
xgb_time<-system.time(xgb1 <- xgboost (data = dtrain, booster = "gbtree", objective = "binary:logistic", eta=0.1, gamma=0.1, max_depth=4, min_child_weight=1, subsample=1, colsample_bytree=1, nrounds = 50))
```

```{r}
xgb_test_time<-system.time(xgb_pred <- predict(xgb1, dtest))
prediction <- as.numeric(pred > 0.5)
err <- mean(as.numeric(pred > 0.5) != dat_test$label)
ac <- roc(factor(xgb_pred), factor(dat_test$label))
auc <- auc(ac)
```

```{r}
cat('The accuracy of xgb model is: ', (1-err)*100, '% \n')
cat('The time of training xgb model is: ', xgb_time[1], 's \n')
