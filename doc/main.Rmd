...
pred_train_GBM_base <- round(test_GBM(fit_train_GBM_base, features = dat_train),digit=0)
...
if(run.test){
  ...
  tm_test_GBM_base <- system.time(pred <- test_GBM(fit_train_GBM_base, dat_test))
}
...
if(run.test){
  ...
  tm_test_GBM_improved <- system.time(pred <- test_GBM(fit_train_GBM_improved, dat_test))
}
...
tpr.fpr <- WeightedROC(label_pred_GBM_improved, label_test, weight_test)
auc_GBM <- WeightedAUC(tpr.fpr)

cat("The AUC of model:  GBM_improved", "is", auc, ".\n")
...
opt.svm <- svm_tune(dat_train, dat_test)

# training rf
tm_svmTrain <- system.time(
  svm_fit <- svm_train(dat_train)
)
save(svm_fit, file = "../output/svm_fit.RData")

# testing rf
tm_svmTest <- system.time(
  svm_test_pred <- svm_test(model = svm_fit,
                          testdf = dat_test)
)
accu <- mean(svm_test_pred == dat_test$label)

# auc rf

ac <- roc(factor(svm_test_pred), factor(dat_test$label))
auc <- auc(ac)

cat("Time for training model SVM = ", tm_svmTrain[1], "s \n")
cat("Time for testing model SVM = ",tm_svmTest[1], "s \n")
cat("The accuracy of SVM is", accu*100, "%.\n")
...
accu <- mean(rf_test_pred == dat_test$label)
...
cat("The accuracy of Random Forest model is", accu*100, "%.\n")
...
params <- list(booster = "gbtree", objective = "binary:logistic", eta=0.1, gamma=0.1, max_depth=4, min_child_weight=1, subsample=1, colsample_bytree=1)
xgb_time<-system.time(xgb1 <- xgb.train (params = params, data = dtrain, nrounds = 50, watchlist = list(train=dtrain, val=dtest), print.every.n = 10, early.stop.round = 10, maximize = F , eval_metric = "error"))
cat('The accuracy of xgb model is: ', (1-xgb1$best_score)*100, '% \n')
cat('The time of training and testing xgb model is: ', xgb_time[1], 's \n')
```
