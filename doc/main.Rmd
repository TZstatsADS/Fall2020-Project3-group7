...
Calculate weightedAUC on testing split
```{r}
label_pred_svm<-as.integer(svm_test)
accu_svm <- sum(weight_test * (label_pred_svm == label_test)) / sum(weight_test)
#prob_pred <- lable_pred
tpr.fpr <- WeightedROC(svm_test, label_test, weight_test)
...
cat("The accuracy of model: XGB", "after reweighting", "is", accu_svm*100, "%.\n")
